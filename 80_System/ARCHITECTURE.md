...
- ~~Ví dụ thứ hai — dự án KHÔNG migrate rules~~ — [[10_Projects/design-system/README]]: dự án có AI toolkit riêng đã trưởng thành, tự khai single source of truth — chỉ index + link, không tạo `ai/`
- ~~ADR đầu tiên chạy thật~~ — `10_Projects/sapo-invoice/migrate-ui-components-sang-design-system.md` (status: Accepted)
- Bug + SRS thật chưa có ví dụ nào — chờ phát sinh từ task thật tiếp theo trên Sapo Invoice/Design System
- Review như artifact bắt buộc của delivery — chưa hình thành thói quen, chờ dùng thực tế

**Done khi:** một dự án mẫu vận hành end-to-end theo Data Flow. Đã có 2 project thật (Sapo Invoice, Design System) và 1 Decision chạy thật; còn thiếu Bug/SRS/Review chạy thật.
...
| v2.9 | 2026-07-29 | Roadmap Phase 4 cập nhật: thêm project Design System (ví dụ không migrate rules), ghi nhận Decision đầu tiên chạy thật (migrate UI library); Bug/SRS/Review vẫn chưa có ví dụ thật |
