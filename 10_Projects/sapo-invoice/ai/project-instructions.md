...
2. Sau khi Duy xác nhận (hoặc không phản đối) → **tự tạo file trực tiếp**, không yêu cầu Duy làm tay:
   ...
   - **Riêng epic có nhiều file liên quan** (SRS + prompt Cursor + test case + file đính kèm...) → gom vào 1 subfolder riêng theo epic, vd `epic-62-cccd/`, `epic-61-phi-van-chuyen-hoa-don-v2/` — mỗi epic 1 folder, không rải các file cùng epic ở ngoài root. Link nội bộ giữa các file trong cùng folder dùng wikilink không kèm path (vd `[[epic-62-test-cases-cccd]]`).
